...
*   **Enlaces de Codigo:**
    *   [package.json (Raiz)](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/package.json): Inicializa los dos workspaces (`api-emitesis` y `web-emitesis`) y orquesta el arranque simultaneo.
...
*   **Enlaces de Codigo:**
    *   [main.ts (Backend)](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/main.ts): Punto de entrada que levanta el servidor en el puerto 5000, e inyecta validaciones globales, interceptores de respuesta y filtros de excepciones.
...
*   **Enlaces de Codigo:**
    *   [middleware.ts (Frontend)](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/web-emitesis/src/middleware.ts): Guardian que intercepta los accesos web del usuario y valida los roles almacenados antes de permitir la visualizacion de las paginas del dashboard.
...
*   **Enlaces de Codigo:**
    *   [schema.prisma](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma): El unico archivo de definicion del modelo de datos e indices relacionales.
...
*   **Accion:** El Coordinador registra una empresa en [Company](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L97) y sube el convenio firmado a [Agreement](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L112).
...
*   **Accion:** El Coordinador crea el expediente de practica vinculando un estudiante, un tutor academico y una empresa en [Internship](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L125).
...
*   **Accion:** Al crearse la asignacion, el backend NestJS consulta las plantillas de documentos obligatorios configurados para la carrera del estudiante en [DocumentTemplate](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L191).
*   **Logica:** Genera automaticamente los registros vacios correspondientes en la tabla [Document](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L261) con el estado `PENDIENTE` y sus respectivas fechas limites (`dueDate`). El estudiante visualiza estas tareas pendientes como un roadmap.
...
1.  **Registro de Asistencia (Check-In / Check-Out):**
    *   Registra coordenadas GPS y adjunta una foto de evidencia que se guarda en la tabla [Attendance](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L310).
...
3.  **Justificacion de Ausencias:**
    *   Si no puede asistir, registra la fecha y motivo de la falta en [Absence](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L336), adjuntando un justificativo digital para revision y aprobacion del Tutor Academico.

### Paso 5: Evaluacion Dual (Rúbricas de Calificacion)
*   **Accion:** Al culminarse las horas planificadas, se disparan dos evaluaciones obligatorias en la tabla [Evaluation](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L210):
...
*   **Logica:** El servicio [certification.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/certification/certification.service.ts) realiza la validacion de elegibilidad. Si se cumplen todos los requisitos, Puppeteer levanta un navegador headless en el servidor, renderiza una plantilla HTML con Handlebars, genera el bufer PDF del certificado, inyecta la firma digital criptografica y un codigo QR unico de verificacion, y cambia el estado del expediente a "Finalizado".

### Paso 7: Politicas LOPDP y Depuracion
*   **Accion:** Si el estudiante requiere ejercer sus derechos de privacidad:
    *   Solicita un reporte en [DataRequest](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/prisma/schema.prisma#L357). El sistema compila y expone sus datos en un JSON de portabilidad.
...
*   **Implementacion en el Servidor:** Ubicado en [webauthn.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/webauthn/webauthn.service.ts). Utiliza la biblioteca `@simplewebauthn/server` para orquestar la generacion de retos criptograficos y validar las respuestas.
*   **Implementacion en el Cliente:** Ubicado en [useWebAuthn.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/web-emitesis/src/hooks/useWebAuthn.ts). Utiliza `@simplewebauthn/browser` para invocar al hardware del sistema operativo. Maneja de forma resiliente excepciones comunes de la API del navegador (`NotAllowedError` si el usuario cancela, `SecurityError` si no se usa HTTPS).
...
*   **Implementacion:** Ubicado en [two-factor-auth.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/auth/two-factor-auth.service.ts).
...
*   **Ubicacion del Codigo:** Metodo `createCheckIn` y `createCheckOut` de [attendance.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/attendance/attendance.service.ts).
*   **Algoritmo Haversine:**
...
*   **Ubicacion del Codigo:** [certification.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/certification/certification.service.ts).
*   **Logica del Flujo:**
...
El modulo de IA en [ai.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/ai/ai.service.ts) implementa servicios de apoyo operativo:
...
*   **Ubicacion del Codigo:** [storage.service.ts](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/infrastructure/storage/storage.service.ts).
...
*   **Script de Configuracion:** [setup-vps.sh](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/scripts/ops/setup-vps.sh) se ejecuta al inicializar el servidor.
...
*   **Servidor Nginx como Proxy Inverso:**
    *   Ubicado en [nginx.conf](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/nginx.conf). Actua como el unico receptor publico de solicitudes del servidor en los puertos 80 y 443.
...
El script [backup-db.sh](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/scripts/ops/backup-db.sh) se ejecuta diariamente en la VPS a las 03:00 AM mediante una tarea del programador de tareas del sistema operativo (Cron Job).
...
"El Indice de Salud se calcula programaticamente en el metodo `calculateHealthScore` de [InternshipsService](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/internships/internships.service.ts). Este unifica 4 criterios ponderados:
...
"Para mitigar ataques de Broken Access Control (OWASP Top 1), el metodo `findOne` de [InternshipsService](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/internships/internships.service.ts) valida estrictamente la relacion de propiedad del recurso. Si un actor no administrativo (Estudiante, Tutor o Empresa) intenta consultar un expediente que no le pertenece, la peticion se deniega arrojando una excepcion `403 Forbidden`. Al mismo tiempo, se dispara un registro de auditoria especializado en SystemLog bajo la categoria `AUTH` con severidad `HIGH` tipo `ID_ENUMERATION`, permitiendo a los administradores identificar de forma inmediata intentos de escaneo maliciosos de la API."
...
"El backend implementa la biblioteca `otplib` para resolver la verificacion del doble factor. En [TwoFactorAuthService](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/auth/two-factor-auth.service.ts), configuramos el validador autenticador estableciendo una configuracion de ventana igual a 10 (`window: 10`) y un paso de 30 segundos (`step: 30`). Esto significa que el servidor tolerara codigos generados hasta 300 segundos antes o despues de la hora oficial del servidor (drift temporal), permitiendo que usuarios con dispositivos desincronizados completen la validacion sin rechazos accidentales."
...
1.  **Provisionamiento (IaC):** Terraform lee [main.tf](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/infra/main.tf), crea la maquina virtual VPS en AWS Lightsail, configura el User Data en bash para aprovisionar Docker y activa el firewall defensivo abriendo unicamente los puertos `22`, `80` y `443`.
...
"NestJS y Prisma manejan la conexion a traves de pools de hilos de red resilientes. Si ocurre un fallo en la conexion con PostgreSQL, Prisma arroja excepciones de conexion especificas (ej. codigos P1001 o P1002). Estas excepciones son capturadas por el filtro global de excepciones [HttpExceptionFilter](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/common/filters/http-exception.filter.ts). Este filtro envia una respuesta estructurada de error HTTP 500 al cliente, evitando fugas de memoria y manteniendo el servicio NestJS en ejecucion para responder a otras rutas locales o solicitudes de cache. Paralelamente, se genera una alerta inmutable en `SystemLog` para que los administradores identifiquen y resuelvan la caida."
...
"El sistema implementa el Dashboard de Monitoreo Predictivo basado en el Indice de Salud (Health Score) y analisis de IA. El Coordinador y Tutor academico pueden ver en sus paneles una lista de pasantes categorizados por nivel de riesgo (Bajo, Medio, Alto). El algoritmo [InternshipsService](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/internships/internships.service.ts) evalua constantemente las variables de avance. Si un estudiante tiene un Health Score bajo, el Tutor puede invocar al modulo de IA en [AiService](file:///home/jorge/Descargas/sistema_practicas_preprofesionales/api-emitesis/src/ai/ai.service.ts) para que analice de forma predictiva las causas exactas (ej: documentos obligatorios no cargados a tiempo o tasa de asistencia deficiente) y recomiende una estrategia de intervencion personalizada al docente."
