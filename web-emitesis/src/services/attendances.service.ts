...
/** Evita `Unexpected end of JSON input` cuando el API devuelve 200 con cuerpo vacío o `null` sin serializar. */
...
export const attendancesService = {
  async getTodayStatus() {
    ...
    if (!res.ok) {
      const errBody = await parseJsonBody<{ message?: string }>(res);
      throw new Error(errBody?.message || 'Error al obtener estado de asistencia');
    }
